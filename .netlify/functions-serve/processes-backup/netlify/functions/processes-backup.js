...
// netlify/functions/processes-backup.js
var processes_backup_exports = {};
__export(processes_backup_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(processes_backup_exports);
...
var handler = async (event) => {
  console.log("\u{1F525} Function called:", event.httpMethod, event.path);
  if (event.httpMethod === "OPTIONS") {
    return { statusCode: 200, headers, body: "" };
  }
  try {
    let path = event.path.replace("/.netlify/functions/analyses", "").replace("/api/analyses", "");
...
//# sourceMappingURL=processes-backup.js.map
